Ext.define('Admin.view.sale.SaleHistoryViewController', {
	extend: 'Ext.app.ViewController',
	alias: 'controller.saleHistoryViewController',
	/********************************************** Controller View *****************************************************/
	/*Add*/
	...
	openEditWindow: function (grid, rowIndex, colIndex) {
		...
		if (record) {
			var win = grid.up('container').add(Ext.widget('saleHistoryEditWindow'));
			win.show();
	...
	quickSearch: function (btn) {
		...
		Ext.apply(store.proxy.extraParams, { saleNum: "", method: "", saleTimeStart: "", saleTimeEnd: "" });

		if (searchField === 'saleNum') {
			Ext.apply(store.proxy.extraParams, { saleNum: searchValue });
		}
		if (searchField === 'saleTime') {
			Ext.apply(store.proxy.extraParams, {
				saleTimeStart: Ext.util.Format.date(searchDateFieldValue, 'Y/m/d H:i:s'),
				saleTimeEnd: Ext.util.Format.date(searchDateFieldValue2, 'Y/m/d H:i:s')
			});
		}
	...
	submitSearchForm: function (btn) {
		var store = Ext.data.StoreManager.lookup('saleHistoryGridStore');
		var win = btn.up('window');
		var form = win.down('form');
	...
	openDetailWindow: function (grid, rowIndex, colIndex) {
		...
		var num = record.get('saleNum');//获取单号
		console.log(num);

		var win = grid.up('container').add(Ext.widget('saleHistoryDetailWindow'));
		win.show();//打开窗口

		//从后台加载数据
		var store = Ext.data.StoreManager.lookup('saleDetailGridStore');
		console.log(store);
		Ext.apply(store.proxy.extraParams, { saleNum: "" });
		Ext.apply(store.proxy.extraParams, {
			saleNum: num,
		});
		store.load({ params: { start: 0, limit: 20, page: 1 } });
	...
	openEditDetailWindow: function (grid, rowIndex, colIndex) {
		...
		//获取选中数据的字段值：console.log(record.get('id')); 或者 console.log(record.data.id);
		if (record) {
			var win = grid.up('container').add(Ext.widget('saleHistoryDetailEditWindow'));
			win.show();
			win.down('form').getForm().loadRecord(record);
		}
	}
